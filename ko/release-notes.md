...
##### Service Gateway
* 사용자 정의 엔드포인트 기능이 추가되었습니다. 사용자가 자신의 로드 밸런서를 엔드포인트로 게시하면, 다른 프로젝트에서 서비스 게이트웨이를 통해 인터넷 경유 없이 접근할 수 있습니다.
	* 사용자 정의 엔드포인트는 한국(평촌) 리전과 한국(판교) 리전에서만 이용할 수 있습니다.
...
* 로드 밸런서 엔진 버전(`v1`/`v2`) 선택 기능이 추가되었습니다. 신규 로드 밸런서는 최신 버전(`v2`)으로 생성되며, 로드 밸런서별로 엔진 버전을 변경할 수 있습니다.                                             
    * 엔진 버전에 따라 HTTP 트래픽 처리 등 일부 동작이 달라질 수 있으므로, 변경 후에는 반드시 검증한 뒤 운영 환경에 적용하시기 바랍니다. 
...
* 같은 프로젝트에서는 여러 VPC에서 하나의 피어 게이트웨이로 연결을 설정할 수 있습니다.
* IKE v2 연결이 지원됩니다.
* VPN 게이트웨이 서비스가 제공되는 리전 간에 VPN 연결이 가능합니다(장비는 Fortinet - Fortigate Series 로 설정).
...
##### Service Gateway
* Service Gateway 생성 시 사용자가 IP 주소를 지정하여 생성할 수 있도록 개선되었습니다.
