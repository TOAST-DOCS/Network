...
##### Load Balancer
* 再暗号化機能が追加されました。メンバーグループのプロトコルとしてHTTP_REENCRYPTを選択すると、メンバーへの送信時にSSL暗号化通信を使用します。
* HTTP Keepaliveの無効化機能が追加されました。Keepaliveタイムアウトの設定時に「使用しない」を選択すると、Keepaliveが無効化されます。
* SSL/TLS暗号化ポリシー設定機能が追加されました。SSL/TLS暗号化スイート(Cipher Suite)をユーザーがカスタマイズして使用できます。
...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (Duplicate '2024. 03. 26.' date heading; ko has only one k70 for this date, and this section's 機能改善/Service Gateway content belongs under k61 (2024.05.28 機能改善) misplaced here) -->
### 2024. 03. 26.
...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (Heading reads '機能変更' but ko k152 is '기능 개선' (機能改善); title mismatch means this cannot be confirmed as a translation — flagged as structurally anomalous extra) -->
